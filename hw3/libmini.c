...
#define WRAPPER_RETvoid(error_msg) \
    errno = 0;                     \
    if (ret < 0) {                 \
        errno = -ret;              \
        perror(error_msg);         \
...
void sigsetjmp(jmp_buf env) {
    ...
    env->mask = old;
    WRAPPER_RETvoid("sigsetjmp error!\n");
}
void siglongjmp(jmp_buf env) {
    // set store mask value
    long ret = sys_rt_sigprocmask(SIG_SETMASK, &(env->mask), NULL, SIGSET_SIZE);
    WRAPPER_RETvoid("siglongjmp error!\n");
}

void *memset(void *addr, int val, size_t size) {
    char *ptr = (char *)addr;
    while (size--) {
        *(ptr)++ = val;
    }
...
int sigismember(const sigset_t *set, int signum) {
    ...
    return (set->val[0] & gen_sigmask(signum)) != 0;
}

int sigprocmask(int how, const sigset_t *set, sigset_t *oldset) {
    long ret = sys_rt_sigprocmask(SIG_BLOCK, set, oldset, SIGSET_SIZE);
    WRAPPER_RETval(int);
}
...
sighandler_t signal(int signum, sighandler_t handler) {
    if (handler == SIG_ERR || signum < 0 || signum >= _NSIG) {
        set_errno(EINVAL);
        return SIG_ERR;
    }
    ...
    if (sigaction(signum, &act, &old_act) < 0) return SIG_ERR;
...
int sigaction(int signum, struct sigaction *act, struct sigaction *old_act) {
    ...
    long ret = sys_rt_sigaction(signum, act, old_act, SIGSET_SIZE);
    WRAPPER_RETval(int);
}
